...
* *New API Subsystems:*
  * Added ``API/wrappers.py`` implementing a Scikit-Learn style interface and remaked the ``GridSearchManager`` object.
...
* *AI & Documentation Integrations:*
  * Added ``LLMs.txt`` (LLM Context File) to provide a strict dependency and architectural map for AI code assistants interacting with the framework.
  * Massive extensions to the Sphinx documentation suite, including quickstart guides, module-specific API breakdowns, and robust autoclass typing.

**Changed / Refactored**

* *Name Changes:*
  * Changed ``SimpleNN`` class name to :class:`~HeteroSymNN.Core.Nets.MLP`.
  * Changed ``FlaxibleNN`` class name to :class:`~HeteroSymNN.Core.Nets.Dense`.
  * Changed ``ConfigurableNN`` class name to :class:`~HeteroSymNN.Core.Nets.HeteroDense`.
...
* *Directory Restructuring:*
  * Moved ``layers.py`` up to the main ``Core/`` directory to separate individual layer logic from full network assemblies.
  * ``docs/`` folder structure overhauled, splitting monolithic files into ``api/``, ``core/``, and ``jit/`` modules.

**Removed**
* Currently `CPU_JIT` method has bee deactivated due to performance issues.
