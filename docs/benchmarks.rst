...
* **JAX Optimization (Anti-Sandbagging):** To give the JAX baseline its optimal "best-case" performance, its execution loops were heavily optimized using top-tier static compilation (`@partial(jax.jit)` and `jax.lax.scan`), ensuring maximum cache hits and completely unrolled C++ execution.
