...
1. **Layers are strictly Memory Managers.** Do not write mathematical physics or activation logic inside a Layer class. Layers only manage ``BackendArray`` allocations and data routing.
2. **Math belongs to the JIT.** All mathematical operations, gradients, and custom loss functions must be parsed symbolically through the ``SymbolicJITCompiler``.
...
`Project Repository <https://github.com/Dilosch03/HeteroSymNN>`