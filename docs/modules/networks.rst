...
* **Feed-Forward / LinearNet Family:** The foundational architecture. Data flows in a single direction from input to output. Perfect for standard classification, regression, and baseline physics approximations.
...
.. toctree::
    ...
    nn_clases/mlp
    nn_clases/linear_net
    nn_clases/hetero_linear_net
...
* **You are mixing and matching layers:** To manually build highly custom network topologies by snapping together layers from different families (e.g., combining standard LinearNet layers with future Recurrent or EvoNet layers). Note that while you can mix layers as you please, there are structural restrictions depending on the specific topologies being connected.
