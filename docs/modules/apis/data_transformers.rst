...
.. note::
   **Extending the Framework:** Need to build a custom data scaler? Jump straight to the :ref:`developer_contract_utilities`.

.. currentmodule:: HeteroSymNN.API.utilities
...
.. automodule:: HeteroSymNN.API.utilities
   :members:
   :undoc-members:
   :show-inheritance:
...
.. _developer_contract_utilities:

Developer Contract: Subclassing Transformers
-----------------------------------------------

If the standard transformers do not fit your data distribution, you can build custom scaling logic by subclassing ``DataTransformer``. You must adhere to the following two lifecycles to integrate safely with the framework's state machine.
...
.. admonition:: Phase 1: The Execution Cycle (Math & State)
   ...
   * **``transform(data)``** & **``inverse_transform(data)``**: Apply and reverse the scaling logic. You must call the ``super()`` equivalents first to trigger the runtime guardrails (which raise a ``RuntimeStateError`` if transformation is attempted before fitting).
