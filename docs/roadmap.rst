...
--------------------------------------------------
* **Lexical Analyzer**: Moving constant extraction and string expansion into the ``SymbolicJITCompiler``.
...
* **Batch dependant constants:** Adding the ability to construct a constant that differed depending on the information in the sample and are batch compatibles.
* **Multi variable loss functions:** Adding the ability to construct a loss function that accepts multiple inputs (e.g., f(x,y,z,w)). This requires expanding the parsing logic.
